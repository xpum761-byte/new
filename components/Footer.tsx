...
export const Footer: React.FC<FooterProps> = ({ onGenerateClick, generationState, buttonText }) => {
  ...
  return (
    <footer className="bg-brand-surface/80 backdrop-blur-sm border-t border-brand-primary/20 p-4 shrink-0">
      <div className="container mx-auto">
        <div className="flex flex-col md:flex-row items-center gap-4">
          <button
            onClick={onGenerateClick}
            disabled={isGenerating}
            className="px-8 py-3 bg-brand-primary text-black font-bold rounded-md hover:opacity-90 transition-all duration-200 disabled:bg-brand-secondary disabled:text-brand-text-muted/50 disabled:cursor-not-allowed shadow-lg shadow-brand-primary/20 flex items-center justify-center w-full md:w-48"
